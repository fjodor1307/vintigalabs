...
function main() {
  ...
  if (!sourceSlug || !newSlug || !/^[a-z0-9][a-z0-9-]*$/.test(newSlug)) {
    console.error('Usage: npm run clone-prototype -- <source-slug> <new-slug>')
  ...
  if (existsSync(changelogPath)) {
    ...
    const entry = `\n## ${date} — Cloned from \`${sourceSlug}\`\n\nCreated as an iteration clone via \`npm run clone-prototype\`.\n`
    appendFileSync(changelogPath, entry)
  }

  console.log(`✓ Cloned src/prototypes/${sourceSlug} → src/prototypes/${newSlug}`)
